...
import { ArrowRight, Code, Globe, Shield, BarChart, Rocket, Server, Zap, Quote } from 'lucide-react';
import { PageRoute } from '../types';
import { useData } from '../context/DataContext';

const Home: React.FC = () => {
  const { siteContent, testimonials } = useData();
      ...
      <section className="bg-indigo-600 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-white">
            <div>
              <div className="text-4xl font-bold mb-2">{siteContent.statsProjects}</div>
              <div className="text-indigo-200">Projects Delivered</div>
            </div>
            <div>
              <div className="text-4xl font-bold mb-2">{siteContent.statsRetention}</div>
              <div className="text-indigo-200">Client Retention</div>
            </div>
            <div>
              <div className="text-4xl font-bold mb-2">{siteContent.statsYears}</div>
      ...
      <section className="py-24 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t) => (
              <div key={t.id} className="bg-slate-50 p-8 rounded-2xl relative border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <Quote className="h-8 w-8 text-indigo-200 absolute top-6 left-6" />
                <p className="text-slate-600 italic mb-6 mt-6 relative z-10 leading-relaxed">
                  "{t.content}"
                </p>
                <div className="flex items-center gap-4">
                  <img src={t.avatar} alt={t.name} className="h-12 w-12 rounded-full object-cover border-2 border-white shadow-sm" />
                  <div>
